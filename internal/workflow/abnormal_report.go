...
func (arh *AbnormalReportHandler) notifyAccountant(ctx context.Context, report *AbnormalReport) error {
	...
	//     Subject: fmt.Sprintf("⚠️  ABNORMAL REIMBURSEMENT REPORT [%s]", report.Severity),
	//     Body:    message,
	// })
...
func (arh *AbnormalReportHandler) printAbnormalReportToConsole(message string, report *AbnormalReport) {
	...
	fmt.Printf("\n")
	fmt.Printf("╔════════════════════════════════════════════════════════════════════╗\n")
	fmt.Printf("║ %s  ABNORMAL REIMBURSEMENT REPORT [%s]                          ║\n", symbol, report.Severity)
	fmt.Printf("╠════════════════════════════════════════════════════════════════════╣\n")
	fmt.Printf("║                                                                    ║\n")

	// Print key details
	fmt.Printf("║  Instance ID:         %-43d  ║\n", report.InstanceID)
	fmt.Printf("║  Item ID:             %-43d  ║\n", report.ItemID)
	fmt.Printf("║  Report Type:         %-43s  ║\n", report.ReportType)
	fmt.Printf("║  Flagged Time:        %-43s  ║\n", report.FlaggedAt.Format("15:04:05"))
	fmt.Printf("║                                                                    ║\n")
	fmt.Printf("║  Confidence Score:    %-6.2f  /  Threshold: %-6.2f              ║\n", report.ConfidenceScore, report.Threshold)

	if report.ConfidenceScore >= report.Threshold {
		fmt.Printf("║  Status:              ✓ ABOVE THRESHOLD (auto-approve ready)         ║\n")
	} else {
		fmt.Printf("║  Status:              ✗ BELOW THRESHOLD (manual review needed)     ║\n")
	}
	...
	fmt.Printf("║                                                                    ║\n")
	fmt.Printf("║  📧 Notification sent to: %-38s  ║\n", report.AccountantEmail)
	fmt.Printf("║                                                                    ║\n")
	fmt.Printf("╚════════════════════════════════════════════════════════════════════╝\n")
	fmt.Printf("\n")
